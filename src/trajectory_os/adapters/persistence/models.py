...
class PortfolioProjectFocusNextReadyTaskDecisionRecordRow(Base):
    """One durable, immutable V1.42 human-accepted next READY TASK decision.

    ``decision_id`` is the immutable identity of EXACTLY this durable
    record (primary key): the same ``decision_id`` may be stored exactly
    once; two value-equivalent V1.42 decisions with different
    ``decision_id`` values are different rows. ``portfolio_id`` is a
    foreign key into ``portfolios.id`` with ``ON DELETE CASCADE`` (same
    convention as the decision tables). ``decided_at`` is the EXACT
    caller-supplied aware ISO-8601 text (UTC offset preserved, never
    normalized). The EXACT accepted V1.42
    ``PortfolioProjectFocusNextReadyTaskDecision`` is stored as
    deterministic explicit JSON produced by the Pydantic JSON
    serialization (``model_dump_json()``) — NOT a pickle and NOT an
    opaque binary: every scalar of the nested decision
    remains independently queryable and visible for corruption. The
    ...
    Append-only: only the V1.43 durable record may INSERT into this
    table; there is no UPDATE/DELETE API for it.
    """

    __tablename__ = (
        "portfolio_project_focus_next_ready_task_decision_records"
    )
...
class TaskExecutionLifecycleDecisionRecordRow(Base):
    """One durable, immutable V1.50 explicit human lifecycle decision record
    (V1.55 record, persisted by V1.56).

    ``decision_record_id`` is the durable identity of exactly this persisted
    historical record. It is a primary key only for durable record identity
    and MUST NOT be interpreted as a decision idempotency key.
    ...
    queryability and corruption visibility. ``lifecycle_decision_id``,
    ``execution_record_id``, and ``authorized_task_id`` deliberately carry
    NO uniqueness and NO foreign keys into replaceable/current tables: the
    same semantic values are legal under distinct ``decision_record_id``
    values, and historical decision records must never be deleted or
    corrupted by replacement of current snapshot state.

    ``disposition`` is stored by its ``TaskExecutionLifecycleDisposition``
    string value. ``decided_at`` and ``recorded_at`` store the exact
    caller-supplied aware ISO-8601 text, preserving each original UTC offset
    (never normalized).

    ``decision_snapshot`` stores deterministic explicit JSON of the exact
    embedded V1.50 ``TaskExecutionLifecycleDecision``. It is never a pickle
    or opaque binary.

    Append-only: V1.56 exposes no update, delete, replace, or upsert path.
    """

    __tablename__ = "task_execution_lifecycle_decision_records"
