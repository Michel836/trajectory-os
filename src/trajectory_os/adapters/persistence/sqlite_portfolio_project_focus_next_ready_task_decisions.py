"""SQLite persistence adapter for durable V1.42 next READY TASK decisions (V1.43).

``SqlitePortfolioProjectFocusNextReadyTaskDecisionRepository`` persists
durable, append-only V1.43 records: ONE SQLite transaction appends EXACTLY
the passed V1.43 ``PortfolioProjectFocusNextReadyTaskDecisionRecord`` into
the immutable ``portfolio_project_focus_next_ready_task_decision_records``
table. There is no update, replace, upsert, save, or delete path: the same
``decision_id`` may be stored EXACTLY ONCE, and
:class:`DuplicatePortfolioProjectFocusNextReadyTaskDecisionError` is raised
for a second attempt — the existing row is never touched.
...
- the EXACT accepted V1.42 decision is stored as deterministic explicit
  JSON via the Pydantic JSON serialization of the genuine nested domain
  object (``model_dump_json()``). This is NOT a pickle and NOT an opaque
  binary blob: every V1.42 scalar also remains independently queryable
  through the dedicated ``portfolio_id`` column (which always agrees with
  the nested decision's ``portfolio_id``, enforced on read) and visible in
  the JSON for corruption checks.
...
back into a genuine, fresh V1.42
``PortfolioProjectFocusNextReadyTaskDecision`` (``model_validate_json`` —
model-level invariants revalidated on every read), and the explicit columns
are re-checked against that genuine decision. The stored ``portfolio_id``
column must agree with the decision's own ``portfolio_id`` (a disagreeing
row is corrupt storage and raises a precise error; no silent adoption).
...
instant, not by the lexical ordering of their stored ISO text) and then by
``decision_id.int`` (numeric UUID order, not lexical string order).
``list_history`` returns ``()`` for an empty history. The reader performs
no derivation and infers no "current"/"effective"/"latest" decision: it
returns exactly the stored records.
"""
...
from trajectory_os.adapters.persistence.models import (
    PortfolioProjectFocusNextReadyTaskDecisionRecordRow as Row,
)
...
class DuplicatePortfolioProjectFocusNextReadyTaskDecisionError(ValueError):
    """Raised when a durable next READY TASK decision with the same decision_id exists."""
...
_DUPLICATE_DECISION_ID_MESSAGE = (
    ...
    "portfolio_project_focus_next_ready_task_decision_records.decision_id"
)


def _is_duplicate_next_ready_task_decision_id_violation(
    exc: IntegrityError,
) -> bool:
...
def _row_to_record(
    ...
    """Reconstruct one strict immutable V1.43 durable record from a row.

    The stored explicit JSON snapshot is FIRST re-validated into a
    genuine, fresh V1.42 ``PortfolioProjectFocusNextReadyTaskDecision``
    through normal strict Pydantic validation (never
    ``model_construct``), and only then is the outer V1.43 record built
    around it. The dedicated ``portfolio_id`` column is then re-checked
    against that genuine decision's own ``portfolio_id``; a disagreeing
    row is corrupt storage and is rejected with a precise error.
    """
    decision = PortfolioProjectFocusNextReadyTaskDecision.model_validate_json(
        row.decision_snapshot,
    )
    if decision.portfolio_id != expected_portfolio_id:
        raise ValueError(
            "stored next READY TASK decision row for decision "
            f"{row.decision_id} carries portfolio_id "
            f"{row.portfolio_id} but its exact V1.42 snapshot declares "
...
class SqlitePortfolioProjectFocusNextReadyTaskDecisionRepository:
    """Append-only durable V1.43 storage plus deterministic read-only history.
    ...
    def add(self, record: object) -> None:
        """Persist one durable V1.43 record with INSERT-only semantics.
        ...
        :class:`DuplicatePortfolioProjectFocusNextReadyTaskDecisionError`.
        ...
        (a hostile nested V1.42 decision that genuine construction could
        ...
        if not isinstance(
            ...
            raise TypeError(
                "record must be a genuine V1.43 "
                "PortfolioProjectFocusNextReadyTaskDecisionRecord "
                f"instance, got {type(record).__name__}"
            )
        ...
        try:
            ...
            session.rollback()
            if _is_duplicate_next_ready_task_decision_id_violation(exc):
                raise (
                    DuplicatePortfolioProjectFocusNextReadyTaskDecisionError(
                        "durable next READY TASK decision already "
                        f"exists: {stored_decision_id}"
                    )
                ) from exc
            raise
        finally:
    ...
    def list_history(
        ...
        """Return the exact durable next READY TASK decision history for one portfolio.
