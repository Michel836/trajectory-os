"""SQLite integration evidence for V1.24 effective effort summaries."""
...
PORTFOLIO_ID = UUID("72111111-1111-4111-8111-111111111111")
PROJECT_ID = UUID("72333333-3333-4333-8333-333333333333")
TASK_CALIBRATED_ID = UUID("72555555-5555-4555-8555-555555555555")
TASK_ORDINARY_ID = UUID("72666666-6666-4666-8666-666666666666")
...
def test_effective_summary_real_sqlite_round_trip_is_read_only_and_deterministic(
    tmp_path: Path,
) -> None:
    """Persist mixed history, reopen, then build the V1.24 summary read-only."""
    database_path = tmp_path / "trajectory-v124.sqlite3"
    ...
    # ------------------------------------------------------------------
    # Fresh repositories: real durable V1.24 read path.
    # ------------------------------------------------------------------
    with (
        SqlitePortfolioRepository(database_path) as portfolio_repository,
        ...
        first = build_effective_work_breakdown_effort_summary_durably(
            portfolio_id=PORTFOLIO_ID,
            project_id=PROJECT_ID,
            portfolio_repository=portfolio_repository,
        ...
        second = build_effective_work_breakdown_effort_summary_durably(
            portfolio_id=PORTFOLIO_ID,
            project_id=PROJECT_ID,
            portfolio_repository=portfolio_repository,
            estimate_reader=estimate_reader,
        ...
        # The pure summary boundary over the same durable plan must agree
        # with the V1.24 durable boundary exactly.
    ...
    assert isinstance(first, WorkBreakdownEffectiveEffortSummary)
    assert first == second
    assert first == summarize_effective_work_breakdown_effort_plan(plan)

    # ------------------------------------------------------------------
    # Exact classification and verbatim root effort arithmetic.
    # Root itself is unestimated; ordinary task 600 s; calibrated task
    # 450 s -> known 1050 s, 2 estimated, 1 unestimated, no complete total.
    # ------------------------------------------------------------------
    ...
    assert first.effort.known_duration_seconds == 1050
    assert first.effort.estimated_entity_count == 2
    assert first.effort.unestimated_entity_count == 1
    assert first.effort.total_duration_seconds is None

    # ------------------------------------------------------------------
    # V1.24 must create neither rows nor schema while building.
    # ------------------------------------------------------------------
